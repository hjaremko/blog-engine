...
pub fn new_post(credentials: Credentials, input: rocket_contrib::json::Json<NewPostRequest>) -> Result<Json<String>, Unauthorized<String>> {
    let request = input.into_inner();

    if credentials.rights != Rights::Administrator {
        return Err(Unauthorized(Option::from("You are not allowed to add new posts".to_string())));
    }
